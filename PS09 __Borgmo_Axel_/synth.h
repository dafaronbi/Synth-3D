...
public:
    ...
    Mod mod;
    double set_mod(double mod_freq);
    double output[FRAMES_PER_BUFFER];
    void init_synth(int num_chan, int samp_rate);
    void execute_cmd(int cmd);
    double *synth_block(int len);
    void add_key(int new_key, double new_freq);
    void rm_key(void);
    void shift_keys(void);
    void init_key(int new_key, double new_freq);
